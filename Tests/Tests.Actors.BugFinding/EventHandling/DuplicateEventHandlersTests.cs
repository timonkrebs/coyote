// Copyright (c) Microsoft Corporation.
// Licensed under the MIT License.

using System.Threading.Tasks;
using Microsoft.Coyote.Tests.Common.Events;
using Xunit;
using Xunit.Abstractions;

namespace Microsoft.Coyote.Actors.BugFinding.Tests
{
    public class DuplicateEventHandlersTests : BaseActorBugFindingTest
    {
        public DuplicateEventHandlersTests(ITestOutputHelper output)
            : base(output)
        {
        }

        [OnEventDoAction(typeof(UnitEvent), nameof(Check1))]
        [OnEventDoAction(typeof(UnitEvent), nameof(Check2))]
        private class A1 : Actor
        {
#pragma warning disable CA1822 // Mark members as static
            private void Check1()
#pragma warning restore CA1822 // Mark members as static
            {
            }

#pragma warning disable CA1822 // Mark members as static
            private void Check2()
#pragma warning restore CA1822 // Mark members as static
            {
            }
        }

        [Fact(Timeout = 5000)]
        public async Task TestDuplicateDoEventHandlerInActor()
        {
            this.TestWithError(r =>
            {
                r.CreateActor(typeof(A1));
            },
            expectedError: "A1 declared multiple handlers for event 'Events.UnitEvent'.");
        }

        private class M1 : StateMachine
        {
            [Start]
            [OnEventDoAction(typeof(UnitEvent), nameof(Check1))]
            [OnEventDoAction(typeof(UnitEvent), nameof(Check2))]
            private class Init : State
            {
            }

#pragma warning disable CA1822 // Mark members as static
            private void Check1()
#pragma warning restore CA1822 // Mark members as static
            {
            }

#pragma warning disable CA1822 // Mark members as static
            private void Check2()
#pragma warning restore CA1822 // Mark members as static
            {
            }
        }

        [Fact(Timeout = 5000)]
        public async Task TestDuplicateDoEventHandlerInStateMachine()
        {
            this.TestWithError(r =>
            {
                r.CreateActor(typeof(M1));
            },
            expectedError: "M1() declared multiple handlers for event 'Events.UnitEvent' in state 'M1+Init'.");
        }

        private class M2 : StateMachine
        {
            [Start]
            [OnEventGotoState(typeof(UnitEvent), typeof(S1))]
            [OnEventGotoState(typeof(UnitEvent), typeof(S2))]
            private class Init : State
            {
            }

            private class S1 : State
            {
            }

            private class S2 : State
            {
            }
        }

        [Fact(Timeout = 5000)]
        public async Task TestDuplicateGotoEventHandlerInStateMachine()
        {
            this.TestWithError(r =>
            {
                r.CreateActor(typeof(M2));
            },
            expectedError: "M2() declared multiple handlers for event 'Events.UnitEvent' in state 'M2+Init'.");
        }

        private class M3 : StateMachine
        {
            [Start]
            [OnEventPushState(typeof(UnitEvent), typeof(S1))]
            [OnEventPushState(typeof(UnitEvent), typeof(S2))]
            private class Init : State
            {
            }

            private class S1 : State
            {
            }

            private class S2 : State
            {
            }
        }

        [Fact(Timeout = 5000)]
        public async Task TestDuplicatePushEventHandlerInStateMachine()
        {
            this.TestWithError(r =>
            {
                r.CreateActor(typeof(M3));
            },
            expectedError: "M3() declared multiple handlers for event 'Events.UnitEvent' in state 'M3+Init'.");
        }

        private class M4 : StateMachine
        {
            [Start]
            private class Init : BaseState
            {
            }

            [OnEventDoAction(typeof(UnitEvent), nameof(Check1))]
            [OnEventDoAction(typeof(UnitEvent), nameof(Check2))]
            private class BaseState : State
            {
            }

#pragma warning disable CA1822 // Mark members as static
            private void Check1()
#pragma warning restore CA1822 // Mark members as static
            {
            }

#pragma warning disable CA1822 // Mark members as static
            private void Check2()
#pragma warning restore CA1822 // Mark members as static
            {
            }
        }

        [Fact(Timeout = 5000)]
        public async Task TestInheritedDuplicateDoEventHandlerInStateMachine()
        {
            this.TestWithError(r =>
            {
                r.CreateActor(typeof(M4));
            },
            expectedError: "M4() inherited multiple handlers for event 'Events.UnitEvent' from state 'M4+BaseState' in state 'M4+Init'.");
        }

        private class M5 : StateMachine
        {
            [Start]
            private class Init : BaseState
            {
            }

            [OnEventGotoState(typeof(UnitEvent), typeof(S1))]
            [OnEventGotoState(typeof(UnitEvent), typeof(S2))]
            private class BaseState : State
            {
            }

            private class S1 : State
            {
            }

            private class S2 : State
            {
            }
        }

        [Fact(Timeout = 5000)]
        public async Task TestInheritedDuplicateGotoEventHandlerInStateMachine()
        {
            this.TestWithError(r =>
            {
                r.CreateActor(typeof(M5));
            },
            expectedError: "M5() inherited multiple handlers for event 'Events.UnitEvent' from state 'M5+BaseState' in state 'M5+Init'.");
        }

        private class M6 : StateMachine
        {
            [Start]
            private class Init : BaseState
            {
            }

            [OnEventPushState(typeof(UnitEvent), typeof(S1))]
            [OnEventPushState(typeof(UnitEvent), typeof(S2))]
            private class BaseState : State
            {
            }

            private class S1 : State
            {
            }

            private class S2 : State
            {
            }
        }

        [Fact(Timeout = 5000)]
        public async Task TestInheritedDuplicatePushEventHandlerInStateMachine()
        {
            this.TestWithError(r =>
            {
                r.CreateActor(typeof(M6));
            },
            expectedError: "M6() inherited multiple handlers for event 'Events.UnitEvent' from state 'M6+BaseState' in state 'M6+Init'.");
        }

        private class M7 : StateMachine
        {
            [Start]
            [OnEventDoAction(typeof(UnitEvent), nameof(Check))]
            [OnEventGotoState(typeof(UnitEvent), typeof(S1))]
            [OnEventPushState(typeof(UnitEvent), typeof(S2))]
            private class Init : State
            {
            }

            private class S1 : State
            {
            }

            private class S2 : State
            {
            }

#pragma warning disable CA1822 // Mark members as static
            private void Check()
#pragma warning restore CA1822 // Mark members as static
            {
            }
        }

        [Fact(Timeout = 5000)]
        public async Task TestDuplicateMixedEventHandlerInStateMachine()
        {
            this.TestWithError(r =>
            {
                r.CreateActor(typeof(M7));
            },
            expectedError: "M7() declared multiple handlers for event 'Events.UnitEvent' in state 'M7+Init'.");
        }
    }
}
